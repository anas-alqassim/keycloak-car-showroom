...
<@layout.registrationLayout displayMessage=messagesPerField.exists('global') displayRequiredFields=true; section>
    ...
    <#elseif section = "description" >
        <h1>${msg("welcomeBack")}</h1>

        <p class="mb-m">
            ${msg("ifAccountLogIn")}
        </p>
        <a class="pf-c-button pf-m-primary btn-lg btn-stroked" href="${url.loginUrl}">${kcSanitize(msg("login"))?no_esc}</a>
    
    <#elseif section = "form">
        <h2 class="mb-m">${msg("createNewAccount")}</h2>
        <form id="kc-register-form" class="${properties.kcFormClass!}" action="${url.registrationAction}" method="post">

            <div id="ngi-toast">
                <img src="${url.resourcesPath}/img/check_circle.svg" alt="checkmark">
                <span>${msg('profileSuccessfullyDeleted')}</span>
                <button type="button">
                    <img src="${url.resourcesPath}/img/close.svg" alt="close">
                </button>
            </div>
